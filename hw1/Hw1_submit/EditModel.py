...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(len(word)):
      corruptLetters = word[i] #the ith character is the corrupted signal
      for alph in EditModel.ALPHABET:
        correctLetters = corruptLetters + alph  #the corrected signal is the corrupt letter + alpahabet added
        correction = word[1:i+1] + alph + word[i+1:]  #so the correction is the concat of the the word (excluding <) till ith char and then alph and then the remaining word
        ret.append(Edit(correction, corruptLetters, correctLetters))
  ...
  def transposeEdits(self, word):
    ...
    ret = []
    #1 is chosen so that the prev char can be used to flip over
    for i in range(1,len(word)):
      corruptLetters = word[i-1:i+1] #corrupt signal need the current and the prev char for the model
      correctLetters = corruptLetters[::-1] #correct signal is the reverse of the corrupt signal
      correction = word[0:i-1] + correctLetters + word[i+1:]
      ret.append(Edit(correction,corruptLetters,correctLetters))  
  ...
  def replaceEdits(self, word):
    ...
    for i in range(len(word)): 
      corruptLetters = word[i]  #the corrupt signal is the current letter
      for alph in EditModel.ALPHABET:
        correctLetters = alph #the correct signal is the added ALPHABET
        correction = word[0:i] + alph + word[i+1:] #the correct is the word until ith char and the alphabet and the remaining word
        if (correction != word):
          ret.append(Edit(correction,corruptLetters,correctLetters))
