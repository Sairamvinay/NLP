...
class NaiveBayes:
    ...
    def __init__(self):
        ...
        self.BETA = 7    #the constant BETA for smoothing in Best Model naive bayes. Can be changed
    ...
    def classify(self, words):
        ...
        if self.naiveBayesBool:
            ...
            #i then use my regular Naive Bayes classifier with smoothing formula same as the binary Bayes on that document

            #NOTES: beta = 7: limit = 5 gives 84%
            self.actualWords = set()    #reset to an empty set always before classifying
    
            for w in words:
                #Pick only those words (without duplicates) which have a minimal count of LIMIT in either of the classifiers
            ...
            for c in self.prior.keys():
                ...
                for w in list(self.actualWords):
                    #boolean naive bayes formula in case of a regular bayes model with feature selection used
